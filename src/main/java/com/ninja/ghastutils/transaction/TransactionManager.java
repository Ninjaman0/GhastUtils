...
public class TransactionManager {
    ...
    public void registerTransaction(Transaction transaction) {
        ...
        ((Map)this.playerTransactions.computeIfAbsent(playerId, (k) -> new HashMap())).put(transaction.getId(), transaction);
        String var10000 = transaction.getId();
        LogManager.debug("Registered transaction: " + var10000 + " for player " + String.valueOf(playerId));
    }

    public Transaction getTransaction(String id) {
        return (Transaction)this.transactions.get(id);
    }

    public Map<String, Transaction> getPlayerTransactions(UUID playerId) {
        return (Map)this.playerTransactions.getOrDefault(playerId, new HashMap());
    }

    public boolean completeTransaction(String id, boolean success, String message) {
        Transaction transaction = (Transaction)this.transactions.get(id);
        if (transaction == null) {
            return false;
        } else {
    ...
    public void cleanup(long maxAge) {
        ...
        for(Map.Entry<String, Transaction> entry : (new HashMap(this.transactions)).entrySet()) {
            ...
            if (transaction.isCompleted() && transaction.getTimestamp() < cutoff) {
                ...
                UUID playerId = transaction.getPlayerId();
                Map<String, Transaction> playerTxs = (Map)this.playerTransactions.get(playerId);
                if (playerTxs != null) {
